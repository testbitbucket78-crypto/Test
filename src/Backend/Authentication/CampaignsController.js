...
const addCampaign = async (req, res) => {
    try {
       ...
       const RetryAndExpirySettings =
        typeof req.body?.RetryAndExpirySettings === 'string'
            ? JSON.parse(req.body.RetryAndExpirySettings)
            : req.body?.RetryAndExpirySettings || {};
...
const copyCampaign = async (req, res) => {
    ...
    if (campaignTitle?.length == 0) {
        CopyQuery = "INSERT INTO Campaign (sp_id,title,channel_id,message_heading,message_content,message_media,message_variables,button_yes,button_no,button_exp,category,time_zone,start_datetime,end_datetime,csv_contacts,segments_contacts,media_type,templateId,buttons,interactive_buttons) SELECT sp_id, CONCAT('copy of ',title),channel_id,message_heading,message_content,message_media,message_variables,button_yes,button_no,button_exp,category,time_zone,' ',' ',csv_contacts,segments_contacts,media_type,templateId,buttons,interactive_buttons FROM Campaign WHERE Id = " + req.params.CampaignId

    } else {
        CopyQuery = `INSERT INTO Campaign (sp_id,title,channel_id,message_heading,message_content,message_media,message_variables,button_yes,button_no,button_exp,category,time_zone,start_datetime,end_datetime,csv_contacts,segments_contacts,media_type,templateId,buttons,interactive_buttons) SELECT sp_id, CONCAT('copy of ${existCampaign[0].title} ', ${randomName}),channel_id,message_heading,message_content,message_media,message_variables,button_yes,button_no,button_exp,category,time_zone,' ',' ',csv_contacts,segments_contacts,media_type,templateId,buttons,interactive_buttons FROM Campaign WHERE Id = ` + req.params.CampaignId
    }
