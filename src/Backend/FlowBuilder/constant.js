...
const insertBot = `INSERT INTO Bots (spid, name, description, channel_id, status, timeout_value, timeout_message, created_by,advanceAction,keywords,updated_at) VALUES ?`;
...
const getAllBots= `SELECT b.id,b.spid,b.name,b.description,b.channel_id,b.status,b.timeout_value,b.timeout_message,b.created_by,b.updated_at,b.created_at,b.published_at,b.deprecated_at,b.isDeleted,b.isDeprecated,b.keywords,b.node_FE_Json,b.advanceAction,b.botVarList,COUNT(bs.id) AS invoked,
...
const deprecateBot= `UPDATE Bots SET isDeprecated=1,status="Deprecated",deprecated_at= NOW() WHERE id=?`;
const geBotSession= `SELECT * FROM BotSessions WHERE botId=? AND spid=? AND started_at BETWEEN ? AND ?`;
...
module.exports = {getGalleryQuery,insertBot,isBotExist,getAllBots,isBotRunning,deleteBot,deprecateBot,updateBotpublishedAt,getBotById,getBotDetailById,insertNode,updateBotStatus,isKeywordUsed,isKeywordUsedSmartReply,updateBots,deleteBotsNode,getSessionsData,insertMessage,geBotSession};