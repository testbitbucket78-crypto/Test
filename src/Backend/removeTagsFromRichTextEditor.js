...
async function getDefaultAttribue(message_variables, spid, customerId) {
  try {
    ...
    for (let i = 0; i < message_variables.length; i++) {
      ...
      } else if(message_variable?.isAttribute == false || message_variable?.isFallback == false){
    ...
    return replaceEmptyValuesInArray(results);
  } catch (err) {
    console.log("Error:", err);
    return [];
  }
...
async function getDynamicURLToBESent(message_variables, spid, customerId) {
  ...
  return replaceEmptyValuesInArray(result);
}
