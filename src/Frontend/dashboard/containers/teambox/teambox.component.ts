import { Component, AfterViewInit, OnInit, ViewChild, ElementRef, HostListener, ViewEncapsulation } from '@angular/core';
	...
	async getAssicatedInteractionData(dataList: any, selectInteraction: any = true) {
		...
		dataList.forEach((item: any) => {
			...
			this.apiService.getAllMessageByInteractionId(item.InteractionId, 'text').subscribe(messageList => {
				item['messageList'] = messageList ? self.groupMessageByDate(messageList) : []
				...
				item['lastMessageReceved'] = self.timeSinceLastMessage(item.lastMessage)
				item['progressbar'] = self.getProgressBar(item.lastMessage)
				item['UnreadCount'] = self.getUnreadCount(item.allmessages)
				console.log(item.allMessage)
				var messageSentCount: any = self.threasholdMessages(item.allmessages)

				console.log(messageSentCount + " messageSentCount " + threasholdMessages)
				threasholdMessages = threasholdMessages + messageSentCount
				self.SIPthreasholdMessages = self.SIPmaxMessageLimt - threasholdMessages
				console.log(self.SIPthreasholdMessages + " SIP THRESHOLD " + threasholdMessages)
